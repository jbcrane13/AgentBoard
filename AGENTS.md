...
- **Dashboard endpoint decision:** chat stays on the Hermes API server (`127.0.0.1:8641`, Bearer, OpenAI-compatible SSE + `/api/sessions/*`) — that remains the right surface for streaming. **Correction (later same day, ADR-023):** the accompanying claim that the `:9119` dashboard's kanban routes were "dashboard-internal" was wrong. They are a documented plugin REST API (`/api/plugins/kanban/*`) and are now used by `HTTPKanbanBackend`. The blast-radius concern stands: a dashboard session also reaches shell-command approval.
...
- **Remote kanban over HTTP (ADR-023, #207):** the Hermes dashboard's bundled kanban plugin (`/api/plugins/kanban/*` on `:9119`) can serve the board, so `HTTPKanbanBackend` is now a second `KanbanDataReading` conformer alongside the SQLite `KanbanDataService`. Two endpoints cover the whole protocol: `GET /board` (columns + `latest_event_id` + server `now`) and `GET /tasks/{id}` (comments, runs, links, events). Both auth modes verified live over Tailscale — gated cookie login and ungated loopback SPA token. Not yet wired into Settings; backend selection by host is the follow-up.
