...
## 2026-08-21 — HTTP write path for remote kanban (ADR-024, closes #207)

- **Protocol parity requires live verification, not mocks.** All HTTP kanban write tests passed against MockURLProtocol (JSON request/response shapes), then a live dashboard cross-check revealed:
  - The create endpoint returns status:"ready" not the drafted status (expected by CLI tests; dashboard auto-promotes freshly created tasks)
  - Single-field PATCH requests work (e.g., assignee:"..." with no status) — the mock did not exercise this
  - Shared HermesDashboardClient auth state prevents double-login when one handler reauthenticates after 401 (shared session cookie survival required)
  - Live testing against loopback before final push would have caught shape mismatches before committing
- **Delegating complex multi-file changes to parallel subagents requires explicit interface contracts.** Three independent slices (dashboard client, HTTP writer, factory/store wiring) were coordinated via a pre-written contract in the task context, not via inter-agent negotiation. Contracts must be final before spawning (no back-and-forth revisions mid-implementation). The contract covered method signatures, return types, error behavior, and shared object lifetime. When a contract mismatch occurs during assembly, spot-check your assumptions by reading the contract, not by asking the agent to re-read it.
- **"Smoke test passes" is ambiguous and dangerous.** App launch-only tests can miss silent failures in new code paths (the HTTP write backend was tested via curl + unit tests, not driven through the UI). When claiming verification, always specify exactly what was exercised: "app launches," "create task via HTTP," "remote profile reselects both backends," etc. A vague "smoke test passes" can hide incomplete coverage.
- **Pre-commit hook catches new test files too.** SwiftFormat's hoistTry and wrapIfStatementBodies rules apply to newly created test files as well as edits; the hook runs across the whole AgentBoard AgentBoardTests scope. Do not hand-edit violations; swiftformat auto-fixes them quickly.
