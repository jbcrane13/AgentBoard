...
AgentBoard is a native **macOS and iOS26 26+** app (Swift 6, SwiftUI) that unifies three AI-assisted dev workflows:

1. **Kanban Board** — GH issues backed issue tracker with CLI refresh + FSEvents live sync
2. **Coding Session Monitor** — tmux session discovery, status, terminal capture, nudge
3. **Hermes Chat** — full chat with session switching, Agent profile switching, gjateway and acp options
